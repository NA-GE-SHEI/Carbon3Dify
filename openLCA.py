...
class OpenLCACalculator:
    ...
    def get_product_system_details(self, product_system_id) -> Dict:
        ...
        try:
            # 直接獲取完整的產品系統對象
            product_system = self.client.get(o.ProductSystem, product_system_id)
    ...
    def run_calculation(self, product_system_id: str, amount: float = 1.0, 
        ...
        try:
            ...
            setup.product_system = o.Ref(id=product_system_id)
            setup.amount = amount
            
            # 如果提供了影響評估方法ID，則使用它
            if impact_method_id:
                setup.impact_method = o.Ref(id=impact_method_id)
