...
class FurnitureDetectionSorter:
    def __init__(self, model_path='yolov10n.pt', confidence_threshold=0.8):
        ...
        self.table_class_id = 60  # COCO數據集中餐桌的類別ID (dining table)
        self.furniture_class_ids = [self.chair_class_id, self.table_class_id]
        
        # 類別名稱映射
        self.class_names = {
            self.chair_class_id: 'Chair',
            self.table_class_id: 'Table'
        }
        
        # ROC曲線數據收集
        self.y_true = []  # 真實標籤 (假設所有圖片都應該檢測到家具)
        self.y_scores = []  # 預測分數 (檢測到家具的最高信心度)
        self.image_names = []  # 圖片名稱
        
        # 家具ID統計數據收集
        self.furniture_stats = {}  # 用於收集每個家具ID的詳細統計
    ...
    def detect_and_classify(self, input_dir):
        ...
        print(f"支援家具類型: Chair (椅子), Table (桌子)")
        print(f"輸出資料夾: {self.base_dir}")
        print("-" * 50)
        
        for furniture_folder in sorted(input_path.iterdir()):
            if furniture_folder.is_dir():
                furniture_id = furniture_folder.name
                print(f"處理家具資料夾: {furniture_id}")
                
                # 為每個家具ID創建對應的輸出資料夾
                furniture_pass_dir = self.pass_dir / furniture_id
                furniture_fail_dir = self.fail_dir / furniture_id
                furniture_pass_original_dir = self.pass_original / furniture_id
                furniture_fail_original_dir = self.fail_original / furniture_id
                
                # 創建家具專屬資料夾
                furniture_pass_dir.mkdir(exist_ok=True)
                furniture_fail_dir.mkdir(exist_ok=True)
                furniture_pass_original_dir.mkdir(exist_ok=True)
                furniture_fail_original_dir.mkdir(exist_ok=True)
                
                # 初始化該家具的統計數據
                self.furniture_stats[furniture_id] = {
                    'total_images': 0,
                    'confidences': [],
                    'success_count': 0,
                    'image_details': [],
                    'detected_types': {}  # 記錄檢測到的家具類型
                }
                
                # 遍歷該家具資料夾內的所有圖片
                for image_path in furniture_folder.iterdir():
                    if image_path.suffix.lower() in image_extensions:
                        total_images += 1
                        self.furniture_stats[furniture_id]['total_images'] += 1
                        ...
                        # 檢查是否檢測到家具
                        furniture_detections = self._get_furniture_detections(results[0])
                        
                        # 收集ROC數據 - 假設所有圖片都應該有家具
                        self.y_true.append(1)  # 真實標籤：這張圖真的有家具
                        self.image_names.append(image_path.name)
                        
                        # 找出最高信心度的家具檢測
                        if furniture_detections:
                            max_confidence = max([det['confidence'] for det in furniture_detections])
                            detected_type = max(furniture_detections, key=lambda x: x['confidence'])['class_name']
                            print(f"    檢測到{detected_type}，最高信心度: {max_confidence:.3f}")
                            
                            # 統計檢測到的家具類型
                            if detected_type not in self.furniture_stats[furniture_id]['detected_types']:
                                self.furniture_stats[furniture_id]['detected_types'][detected_type] = 0
                            self.furniture_stats[furniture_id]['detected_types'][detected_type] += 1
                            
                            # 收集家具統計數據
                            self.furniture_stats[furniture_id]['confidences'].append(max_confidence)
                            
                            # 預測分數：檢測到家具的最高置信度
                            self.y_scores.append(max_confidence)
                            
                            if max_confidence >= self.confidence_threshold:
                                self.furniture_stats[furniture_id]['success_count'] += 1
                                pass_count += 1
                                print(f"    -> 辨識成功")
                                print(f"       原始圖片複製到: {furniture_pass_original_dir}")
                                print(f"       裁剪圖片保存到: {furniture_pass_dir}")
                                
                                # 複製原始圖片到pass/original_img/{furniture_id}/，裁剪家具區域保存到pass/{furniture_id}/
                                self._copy_image(image_path, furniture_pass_original_dir)
                                self._save_cropped_furniture(original_image, furniture_detections, 
                                                           image_path.name, max_confidence, furniture_pass_dir)
                                
                                # 記錄圖片詳細資訊
                                self.furniture_stats[furniture_id]['image_details'].append({
                                ...
                                fail_count += 1
                                print(f"    -> 辨識失敗（信心度過低）")
                                print(f"       原始圖片複製到: {furniture_fail_original_dir}")
                                print(f"       帶標籤圖片保存到: {furniture_fail_dir}")
                                
                                # 複製原始圖片到fail/original_img/{furniture_id}/，保存帶標籤圖片到fail/{furniture_id}/
                                self._copy_image(image_path, furniture_fail_original_dir)
                                self._save_labeled_image(original_image, results[0], image_path.name, furniture_fail_dir)
                            ...
                            self.furniture_stats[furniture_id]['confidences'].append(0.0)
        ...
        # 生成家具分析報告 - 保存到image_identify資料夾中
        self._generate_furniture_analysis_report()
        
        # 進行置信度分析（更適合當前使用情境）
        self._analyze_confidence_distribution()
        
        # 繪製精準度-召回率曲線（更適合家具檢測任務）
        ...
        if self._has_negative_samples():
            ...
            print("\n" + "💡 " + "="*50)
            print("關於ROC曲線的說明：")
            print("由於所有圖片都是家具圖片（正樣本），無法繪製傳統ROC曲線")
            print("❌ 不建議將低置信度結果當作負樣本，因為：")
            print("   1. 會造成循環邏輯（用閾值定義標籤，再評估閾值）")
            print("   2. 低置信度的家具圖片仍然是家具")
            print("   3. 會產生誤導性的評估結果")
    ...
    def _get_furniture_detections(self, result):
        """
        獲取家具的檢測結果（椅子和桌子）
        ...
        if result.boxes is not None:
            ...
            for i, class_id in enumerate(classes):
                if int(class_id) in self.furniture_class_ids:
                    class_name = self.class_names.get(int(class_id), 'Unknown')
                    furniture_detections.append({
    ...
    def _save_cropped_furniture(self, image, furniture_detections, filename, max_confidence, output_dir):
        """
        保存裁剪的家具區域
        
        Args:
            image: 原始圖片
            furniture_detections: 家具檢測結果
            filename: 檔案名稱
            max_confidence: 最高信心度
            output_dir: 輸出資料夾路徑
        """
        # 找到信心度最高的家具
        best_furniture = max(furniture_detections, key=lambda x: x['confidence'])
        bbox = best_furniture['bbox'].astype(int)
        furniture_type = best_furniture['class_name']
        ...
        cropped_furniture = image[y1:y2, x1:x2]
        
        # 生成新檔名（包含家具類型和精度資訊）
        name_parts = Path(filename).stem, furniture_type, f"conf_{max_confidence:.3f}", Path(filename).suffix
        new_filename = f"{name_parts[0]}_{name_parts[1]}_{name_parts[2]}{name_parts[3]}"
        
        # 保存裁剪的家具圖片到指定的家具資料夾
        save_path = output_dir / new_filename
        cv2.imwrite(str(save_path), cropped_furniture)
        
        print(f"      裁剪區域: ({x1},{y1}) -> ({x2},{y2})")
        print(f"      家具類型: {furniture_type}")
    ...
    def _save_labeled_image(self, image, result, filename, output_dir):
        ...
        if result.boxes is not None:
            ...
            for i, (class_id, conf, bbox) in enumerate(zip(classes, confidences, xyxy)):
                ...
                # 選擇顏色（家具用紅色，其他用綠色）
                color = (0, 0, 255) if int(class_id) in self.furniture_class_ids else (0, 255, 0)
    ...
    def _print_summary(self, total, pass_count, fail_count):
        ...
        print(f"辨識失敗 (<{self.confidence_threshold} 或未檢測到家具): {fail_count}")
        ...
        print(f"   │   │   ├── Table1/ (原始成功圖片)")
        print(f"   │   │   └── ...")
        print(f"   │   ├── Chair1/ (裁剪成功圖片)")
        print(f"   │   ├── Table1/ (裁剪成功圖片)")
        print(f"   │   └── ...")
        print(f"   └── identify_fail/")
        print(f"       ├── original_img/")
        print(f"       │   ├── Chair1/ (原始失敗圖片)")
        print(f"       │   ├── Table1/ (原始失敗圖片)")
        print(f"       │   └── ...")
        print(f"       ├── Chair1/ (帶標籤失敗圖片)")
        print(f"       ├── Table1/ (帶標籤失敗圖片)")
        print(f"       └── ...")
    ...
    def _plot_roc_curve(self):
        ...
        plt.title('Furniture Detection ROC Curve Analysis', fontsize=14, fontweight='bold')
    ...
    def _generate_furniture_analysis_report(self):
        """
        生成詳細的家具分析報告並保存到image_identify資料夾
        """
        if not self.furniture_stats:
            print("沒有家具統計數據可生成報告")
            return
        ...
        for furniture_id, stats in sorted(self.furniture_stats.items()):
            if stats['total_images'] > 0:
                ...
                # 統計檢測到的家具類型
                detected_types_str = ', '.join([f"{k}({v})" for k, v in stats['detected_types'].items()]) if stats['detected_types'] else 'None'
        ...
        # 生成控制台報告
        print(f"{'家具ID':<10} {'照片數量':<8} {'平均置信度':<12} {'最高置信度':<12} {'檢測成功率':<12} {'成功數量':<8} {'檢測類型':<20}")
        print("-"*100)
        ...
        for data in report_data:
            print(f"{data['furniture_id']:<10} {data['total_images']:<8} "
                  f"{data['avg_confidence']:<12.3f} {data['max_confidence']:<12.3f} "
                  f"{data['success_rate']:<12.1f}% {data['success_count']:<8} {data['detected_types']:<20}")
            
            total_images_all += data['total_images']
            total_success_all += data['success_count']
            
            # 收集所有置信度
            furniture_confidences = self.furniture_stats[data['furniture_id']]['confidences']
            all_confidences.extend(furniture_confidences)
        ...
        print(f"{'總計':<10} {total_images_all:<8} "
              ...
              f"{overall_success_rate:<12.1f}% {total_success_all:<8} {'Mixed':<20}")
        ...
        # 找出表現最好和最差的家具
        self._analyze_furniture_performance(report_data)
        
    def _save_csv_report(self, report_data):
        """
        保存CSV格式的家具分析報告到image_identify資料夾
        """
        csv_filename = self.base_dir / 'furniture_analysis_report.csv'
        
        with open(csv_filename, 'w', newline='', encoding='utf-8-sig') as csvfile:
            fieldnames = ['Furniture ID', 'Image Count', 'Avg Confidence', 'Max Confidence', 'Success Rate(%)', 'Success Count', 'Detected Types']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            
            writer.writeheader()
            for data in report_data:
                writer.writerow({
                    'Furniture ID': data['furniture_id'],
                    ...
                    'Detected Types': data['detected_types']
                })
    ...
    def _save_detailed_report(self, report_data):
        ...
        """
        report_filename = self.base_dir / 'furniture_detailed_analysis.txt'
        
        with open(report_filename, 'w', encoding='utf-8') as f:
            f.write("Furniture Detection Detailed Analysis Report\n")
            f.write("="*60 + "\n\n")
            
            f.write(f"Detection Parameters:\n")
            f.write(f"- Model: YOLOv10\n")
            f.write(f"- Confidence Threshold: {self.confidence_threshold}\n")
            f.write(f"- Supported Furniture Types: Chair, Table\n")
            ...
            f.write(f"- Total Furniture Count: {len(report_data)}\n")
            ...
            # 各家具詳細資訊
            f.write("Detailed Analysis by Furniture:\n")
            f.write("-"*60 + "\n")
            ...
            for data in report_data:
                ...
                f.write(f"  Detected Types: {data['detected_types']}\n")
                
                # 顯示每張圖片的詳細資訊
                if furniture_id in self.furniture_stats:
                    f.write(f"  Image Details:\n")
                    for img_detail in self.furniture_stats[furniture_id]['image_details']:
                        f.write(f"    - {img_detail['image_name']}: "
                               f"Type={img_detail['detected_type']}, "
                               f"Confidence={img_detail['confidence']:.3f}, "
                               f"Result={img_detail['result']}\n")
        
        print(f"✅ Detailed report saved to: {report_filename}")
        
    def _analyze_furniture_performance(self, report_data):
        """
        分析家具檢測效果
        """
        if not report_data:
        ...
        # 最佳表現
        best_furniture = sorted_by_success[0]
        print(f"🥇 檢測成功率最高: {best_furniture['furniture_id']} ({best_furniture['success_rate']:.1f}%)")
        
        # 最差表現
        worst_furniture = sorted_by_success[-1]
        print(f"🔴 檢測成功率最低: {worst_furniture['furniture_id']} ({worst_furniture['success_rate']:.1f}%)")
        
        # 按平均置信度排序
        sorted_by_confidence = sorted(report_data, key=lambda x: x['avg_confidence'], reverse=True)
        highest_conf_furniture = sorted_by_confidence[0]
        print(f"💪 平均置信度最高: {highest_conf_furniture['furniture_id']} ({highest_conf_furniture['avg_confidence']:.3f})")
        
        # 需要關注的家具（成功率低於80%）
        low_performance_furniture = [data for data in report_data if data['success_rate'] < 80]
        if low_performance_furniture:
            print(f"\n⚠️  需要關注的家具 (成功率 < 80%):")
            for furniture in low_performance_furniture:
                print(f"   {furniture['furniture_id']}: {furniture['success_rate']:.1f}% - {furniture['detected_types']}")
        else:
            print(f"\n✅ 所有家具檢測成功率都 ≥ 80%")
        
        # 統計檢測到的家具類型分布
        all_detected_types = {}
        for furniture_id, stats in self.furniture_stats.items():
            for detected_type, count in stats['detected_types'].items():
                if detected_type not in all_detected_types:
                    all_detected_types[detected_type] = 0
                all_detected_types[detected_type] += count
        
        if all_detected_types:
            print(f"\n📊 檢測到的家具類型統計:")
            for furniture_type, count in sorted(all_detected_types.items()):
                print(f"   {furniture_type}: {count} 次檢測")
    ...
    def _plot_precision_recall_curve(self):
        """
        繪製精準度-召回率曲線（適合家具檢測任務）並保存到image_identify資料夾
        """
        if len(self.y_scores) == 0:
            print("No detection data for analysis")
            return
        ...
        for threshold in thresholds:
            # 在此閾值下檢測為家具的圖片數
            detected_as_furniture = np.sum(y_scores >= threshold)
            total_furniture = len(y_scores)  # 所有圖片都是家具
            
            # 計算精準度和召回率
            if detected_as_furniture > 0:
                precision = 1.0  # 檢測為家具的都確實是家具
                recall = detected_as_furniture / total_furniture  # 成功檢測的家具比例
            else:
                precision = 0.0
                recall = 0.0
        ...
        plt.xlim([0.0, 1.0])
        plt.ylim([0.0, 1.05])
        plt.xlabel('Recall - Proportion of Successfully Detected Furniture', fontsize=12)
        plt.ylabel('Precision - Accuracy of Detection Results', fontsize=12)
        plt.title('Furniture Detection Precision-Recall Curve', fontsize=14, fontweight='bold')
        ...
        explanation_text += "• Higher recall means more furniture detected\n"
...
def main():
    ...
    # 創建檢測器
    detector = FurnitureDetectionSorter(
